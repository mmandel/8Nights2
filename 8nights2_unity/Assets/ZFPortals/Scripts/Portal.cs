...
public class Portal : MonoBehaviour {
	...
	protected class PhysicalTrackingInfo {
		 ...
		 * (We don't want to teleport things that hit us form behind.)
		 */
		public bool hasBeenInFront = false;

		/**
		 * Time.fixedTime for the last time we processed this. Needed because a root can have multiple
	...
	public void OnTriggerStay(Collider otherCollider) {
		...
		var pos = transform.InverseTransformPoint(trackingInfo.center.position);
		var isInFront = pos.z <= 0;

		trackingInfo.hasBeenInFront = trackingInfo.hasBeenInFront || isInFront;

		//How much closer are we?
		// var lastDistance = trackingInfo.lastDistance;
		// var currentDistance = PortalMath.DistanceFromPointToPlane(transform.position, transform.forward, other.transform.position);
		// var distanceDelta = currentDistance - lastDistance;
		// trackingInfo.lastDistance = currentDistance;
//distance stuff remove

		//If it was once in front of us and now it's behind we should look into teleporting it.
		if (trackingInfo.hasBeenInFront && !isInFront) {
			TeleportIfNeeded(trackingInfo.root);
		}
	...
	protected void TeleportIfNeeded(Transform obj) {
		//Is teleporting this something we should do?
	...
	public void TeleportObjectThroughPortal(Transform obj) {
		...
		if (characterMotor) {
			//CharacterMotor isn't available from our compilation unit so integrate with it using...reflection. :-(
