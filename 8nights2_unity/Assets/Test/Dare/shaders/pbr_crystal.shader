...
///////// Gloss:
                float4 node_3279 = _Time + _TimeEditor;
                float2 node_3938 = ((i.uv0+node_3279.g*float2(0,0))*float2(2,1));
...
////// Specular:
                ...
                float node_7483_ang = node_3279.g;
                ...
                float2 node_4303 = (_facets_var.rgb.rg+node_3279.g*float2(0.01,0.02));
...
////// Emissive:
                float2 node_2436 = (float2(0,0)+node_3279.g*float2(0.4,0.4));
                float4 _flickergradient_var = tex2D(_flickergradient,TRANSFORM_TEX(node_2436, _flickergradient));
                float node_9018 = ((((1.0 - i.uv0.g)*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection)))))*(_flickergradient_var.r*0.1+0.0))*_overbloom);
...
////// Lighting:
                ...
                float InvPi = 0.31830988618;
///////// Gloss:
                float4 node_1655 = _Time + _TimeEditor;
                float2 node_3938 = ((i.uv0+node_1655.g*float2(0,0))*float2(2,1));
                float4 _noise_copy = tex2D(_Noise,TRANSFORM_TEX(node_3938, _Noise));
                float gloss = 1.0 - saturate(( (_noise_copy.r*0.2+0.45) > 0.5 ? (1.0-(1.0-2.0*((_noise_copy.r*0.2+0.45)-0.5))*(1.0-_Roughness)) : (2.0*(_noise_copy.r*0.2+0.45)*_Roughness) )); // Convert roughness to gloss
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                float NdotL = max(0, dot( normalDirection, lightDirection ));
                float LdotH = max(0.0,dot(lightDirection, halfDirection));
                float node_7483_ang = node_1655.g;
                float node_7483_spd = 0.01;
                float node_7483_cos = cos(node_7483_spd*node_7483_ang);
                float node_7483_sin = sin(node_7483_spd*node_7483_ang);
                float2 node_7483_piv = float2(0.5,0.5);
                ...
                float2 node_7483 = (mul(node_5159-node_7483_piv,float2x2( node_7483_cos, -node_7483_sin, node_7483_sin, node_7483_cos))+node_7483_piv);
                float4 _facets_var = tex2D(_facets,TRANSFORM_TEX(node_7483, _facets));
                float2 node_4303 = (_facets_var.rgb.rg+node_1655.g*float2(0.01,0.02));
                float4 node_3372 = tex2D(_Noise,TRANSFORM_TEX(node_4303, _Noise));
                float3 diffuseColor = (_Color.rgb*node_3372.g); // Need this for specular when using metallic
                float specularMonochrome;
                float3 specularColor;
...
/////// Vectors:
                ...
                float2 node_2436 = (float2(0,0)+node_9206.g*float2(0.4,0.4));
                float4 _flickergradient_var = tex2D(_flickergradient,TRANSFORM_TEX(node_2436, _flickergradient));
                float node_9018 = ((((1.0 - i.uv0.g)*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection)))))*(_flickergradient_var.r*0.1+0.0))*_overbloom);
                o.Emission = float3(node_9018,node_9018,node_9018);
                
                float node_7483_ang = node_9206.g;
                float node_7483_spd = 0.01;
                float node_7483_cos = cos(node_7483_spd*node_7483_ang);
                float node_7483_sin = sin(node_7483_spd*node_7483_ang);
                float2 node_7483_piv = float2(0.5,0.5);
                ...
                float2 node_7483 = (mul(node_5159-node_7483_piv,float2x2( node_7483_cos, -node_7483_sin, node_7483_sin, node_7483_cos))+node_7483_piv);
                float4 _facets_var = tex2D(_facets,TRANSFORM_TEX(node_7483, _facets));
                float2 node_4303 = (_facets_var.rgb.rg+node_9206.g*float2(0.01,0.02));
                float4 node_3372 = tex2D(_Noise,TRANSFORM_TEX(node_4303, _Noise));
                float3 diffColor = (_Color.rgb*node_3372.g);
                float specularMonochrome;
                float3 specColor;
                float2 node_3938 = ((i.uv0+node_9206.g*float2(0,0))*float2(2,1));
                float4 _noise_copy = tex2D(_Noise,TRANSFORM_TEX(node_3938, _Noise));
