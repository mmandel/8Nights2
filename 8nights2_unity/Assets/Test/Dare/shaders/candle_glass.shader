...
///////// Gloss:
                ...
                float2 node_3938 = ((i.uv0+node_9123.g*float2(0,0))*float2(2,1));
...
////// Specular:
                ...
                float node_7483_ang = node_9123.g;
                ...
                float node_9844 = 3.0;
                float node_6972 = 1.0;
                float2 node_5159_tc_rcp = float2(1.0,1.0)/float2( node_9844, 3.0 );
                ...
                float2 node_4303 = (_facets_var.rgb.rg+node_9123.g*float2(0.01,0.02));
                float4 node_3372 = tex2D(_Noise,TRANSFORM_TEX(node_4303, _Noise));
                float3 node_1935 = (_Color.rgb*node_3372.g);
...
////// Emissive:
                ...
                float2 node_2436 = (float2(0,0)+node_9123.g*float2(0.4,0.4));
                float4 _flickergradient_var = tex2D(_flickergradient,TRANSFORM_TEX(node_2436, _flickergradient));
                float3 emissive = ((((lerp(float3(1,1,1),saturate(3.0*abs(1.0-2.0*frac((node_9473.r+(_coloroffset*node_3372.r))+float3(0.0,-1.0/3.0,1.0/3.0)))-1),node_9473.r)*node_9473.r)*(1.0 - i.uv0.g)*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection)))))*_overbloom*node_1935)+(_flickergradient_var.r*0.01+0.0));
/// Final Color:
...
////// Lighting:
                ...
                float InvPi = 0.31830988618;
///////// Gloss:
                float4 node_8346 = _Time + _TimeEditor;
                float2 node_3938 = ((i.uv0+node_8346.g*float2(0,0))*float2(2,1));
                float4 _noise_copy = tex2D(_Noise,TRANSFORM_TEX(node_3938, _Noise));
                float gloss = 1.0 - saturate(( (_noise_copy.r*0.2+0.45) > 0.5 ? (1.0-(1.0-2.0*((_noise_copy.r*0.2+0.45)-0.5))*(1.0-_Roughness)) : (2.0*(_noise_copy.r*0.2+0.45)*_Roughness) )); // Convert roughness to gloss
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                float NdotL = max(0, dot( normalDirection, lightDirection ));
                float LdotH = max(0.0,dot(lightDirection, halfDirection));
                float node_7483_ang = node_8346.g;
                float node_7483_spd = 0.01;
                float node_7483_cos = cos(node_7483_spd*node_7483_ang);
                float node_7483_sin = sin(node_7483_spd*node_7483_ang);
                float2 node_7483_piv = float2(0.5,0.5);
                ...
                float2 node_7483 = (mul(node_5159-node_7483_piv,float2x2( node_7483_cos, -node_7483_sin, node_7483_sin, node_7483_cos))+node_7483_piv);
                float4 _facets_var = tex2D(_facets,TRANSFORM_TEX(node_7483, _facets));
                float2 node_4303 = (_facets_var.rgb.rg+node_8346.g*float2(0.01,0.02));
                float4 node_3372 = tex2D(_Noise,TRANSFORM_TEX(node_4303, _Noise));
                float3 node_1935 = (_Color.rgb*node_3372.g);
                float3 diffuseColor = node_1935; // Need this for specular when using metallic
                float specularMonochrome;
...
/////// Vectors:
                ...
                float node_7483_ang = node_1397.g;
                float node_7483_spd = 0.01;
                float node_7483_cos = cos(node_7483_spd*node_7483_ang);
                float node_7483_sin = sin(node_7483_spd*node_7483_ang);
                float2 node_7483_piv = float2(0.5,0.5);
                ...
                float2 node_7483 = (mul(node_5159-node_7483_piv,float2x2( node_7483_cos, -node_7483_sin, node_7483_sin, node_7483_cos))+node_7483_piv);
                float4 _facets_var = tex2D(_facets,TRANSFORM_TEX(node_7483, _facets));
                float2 node_4303 = (_facets_var.rgb.rg+node_1397.g*float2(0.01,0.02));
                float4 node_3372 = tex2D(_Noise,TRANSFORM_TEX(node_4303, _Noise));
                float3 node_1935 = (_Color.rgb*node_3372.g);
                float2 node_2436 = (float2(0,0)+node_1397.g*float2(0.4,0.4));
                float4 _flickergradient_var = tex2D(_flickergradient,TRANSFORM_TEX(node_2436, _flickergradient));
                o.Emission = ((((lerp(float3(1,1,1),saturate(3.0*abs(1.0-2.0*frac((node_9473.r+(_coloroffset*node_3372.r))+float3(0.0,-1.0/3.0,1.0/3.0)))-1),node_9473.r)*node_9473.r)*(1.0 - i.uv0.g)*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection)))))*_overbloom*node_1935)+(_flickergradient_var.r*0.01+0.0));
                
                float3 diffColor = node_1935;
                float specularMonochrome;
                float3 specColor;
                float2 node_3938 = ((i.uv0+node_1397.g*float2(0,0))*float2(2,1));
                float4 _noise_copy = tex2D(_Noise,TRANSFORM_TEX(node_3938, _Noise));
